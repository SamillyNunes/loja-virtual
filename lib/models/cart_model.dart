...
class CartModel extends Model {
  ...
  String couponCode; //codigo do cupom
  int discountPercentage=0; //porcentagem de desconto
  ...
  void addCartItem(CartProduct cartProduct){
    ...
    Firestore.instance.collection("users").document(user.firebaseUser.uid)
      .collection("cart").add(cartProduct.toMap()).then((doc){
        cartProduct.cid=doc.documentID; //pega o id que o proprio firebase da
    });
  ...
  Future<String> finishOrder() async{
    ...
    DocumentReference refOrder= await Firestore.instance.collection("orders").add(
      {
        ...
        "discount":discount,
        "totalPrice":productsPrice-discount+ship,
        "status":1
      }
    );
    ...
    //deletando os arquivos do carrinho
  ...
  void _loadCartItems() async{
    QuerySnapshot query = await Firestore.instance.collection("users").document(user.firebaseUser.uid).collection("cart")
        .getDocuments(); //obtendo os documentos

    products=query.documents.map((doc) => CartProduct.fromDocument(doc)).toList(); //retornando uma lista com todos esses
